...
def test_patch_and_write_sets_gtk_xft_dpi_from_non_default_text_scaling_factor(
    ...
    monkeypatch.setattr(
        ...
        lambda key, schema=module._INTERFACE_SCHEMA: {
            ...
            (module._INTERFACE_SCHEMA, "text-scaling-factor"): "1.25",
    ...
    assert result.get("Settings", "gtk-xft-dpi") == "122880"
...
def test_patch_and_write_sets_gtk_xft_dpi_to_default_when_scaling_is_default(
    monkeypatch, tmp_path: Path
) -> None:
    base_path = tmp_path / "settings.ini"
    output_path = tmp_path / "output.ini"
    ...
    assert result.get("Settings", "gtk-xft-dpi") == "-1"
