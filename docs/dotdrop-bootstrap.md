...
runs the repo's `dotman install`.
...
- After bootstrapping, the script runs the repo-local `dotfiles/bin/dotman install`
...
Pass any other arguments through to `dotman install`, for example:
