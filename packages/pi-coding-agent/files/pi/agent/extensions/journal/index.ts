...
function oneLineSnippet(value: string, maxLength = 180): string {
  ...
  pi.registerTool(
    defineTool({
      ...
      async execute(_toolCallId, params, signal) {
        ...
        return {
          content: [{ type: "text", text: `logged: ${path}\n${oneLineSnippet(params.journal)}` }],
          details: { path, author, highlight: params.highlight, journal: params.journal } satisfies JournalWriteDetails,
        };
      },

      renderCall(_args, theme, context) {
        const path = (context.state as JournalRenderState).path;
        const text = path
          ? `${theme.fg("toolTitle", theme.bold("journal"))} ${theme.fg("accent", path)}`
          : theme.fg("toolTitle", theme.bold("journal"));
      ...
      renderResult(result, { expanded }, theme, context) {
        ...
        if (!details) {
          ...
          return new Text(content?.type === "text" ? content.text : "journal logged", 0, 0);
        }

        const state = context.state as JournalRenderState;
        if (state.path !== details.path) {
          state.path = details.path;
          context.invalidate();
        }

        const body = expanded ? details.journal : oneLineSnippet(details.journal);
