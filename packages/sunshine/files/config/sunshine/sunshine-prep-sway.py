...
"""
Prepare Sway outputs for Sunshine streaming, then restore them on cleanup.

This script is intended to be used via Sunshine's `global_prep_cmd`.

Usage:
  sunshine-prep-sway.py do --width WIDTH --height HEIGHT --fps FPS [--output OUTPUT] [--solo] [--scale SCALE] [--inhibit]
  sunshine-prep-sway.py undo
...
def compute_scale(
    ...
    *,
    ...
    if low in {"auto", "heuristic", "client-auto", "dpi-auto"}:
        ...
        if dpi is not None and TARGET_DPI > 0:
            ...
            return (
                formatted,
                (
                    f"[sunshine-prep-sway] scale mode={low} "
    ...
    try:
        ...
        return (
            formatted,
            f"[sunshine-prep-sway] scale mode=manual requested={scale_arg} applied_scale={formatted}",
        )
    except Exception:
        return (
            None,
            f"[sunshine-prep-sway] scale mode=invalid requested={scale_arg!r} (ignored)",
        )
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sway output prep for Sunshine")
    ...
    p_do.add_argument(
        "--inhibit",
        action="store_true",
        help="Renew Noctalia idle inhibitor lease when available",
    )

    sub.add_parser("undo", help="Re-enable currently connected outputs and clear idle inhibitor")
