...
Metadata safety: `Highlight` becomes frontmatter `aliases`. Use the helper instead of hand-writing journal files; it YAML-quotes the alias before QuickAdd so colons like `Reasoning graph: ...` do not break metadata. Obsidian may later normalize safe quoted scalars back to unquoted YAML; that is OK if frontmatter still parses. If manually fixing/editing journal frontmatter, quote alias values that contain `:` or other YAML-significant chars.
